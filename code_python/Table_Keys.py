        ...
        for bloc in L_bloc:
            ...
            if len(L) > 2:
                if L[1] != "Table où est stockée la data : ''":
                    ...
                    Liste = L[2].split("'")
                    Faux_Nom_clé = f"'{Liste[1]}'"
                    Nom_clé = Faux_Nom_clé.replace(" ", "_")
                    Liste = L[3].split("'")
                    Description = f""
                    ...
                    for k in range(len(L)):
                        if k != 0 and k != 9:
                            cle_restante += f"{L[k]}\n"
                    cle_restante += "--------------------------------------------------------------------------------------\n"
    with open("../ProjetKarmineCorp/fichier_clé_txt/clés_Match_restantes.txt", "w") as f:
...
def cree_table_cle_pleine(cursor):
    ...
    rempli_table_clés(cursor, "../ProjetKarmineCorp/fichier_clé_txt/param_clés.txt", "Match_JSON")


#################Ajout de nouvelles clés à partir d'un fichier txt bien formaté#####################################################################################################################

def rajoute_cle(cursor, nouvelle_cle_txt, type_cle):
    ...
    """
    rempli_table_clés(cursor, nouvelle_cle_txt, type_cle)
